...
resource "null_resource" "kubeadm_init" {
  ...
  provisioner "local-exec" {
    command = "scp -i env/$${INSTANCE}/id_rsa_core -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o \"ProxyCommand ssh -W %h:%p -i env/$${INSTANCE}/id_rsa_core -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no core@${var.bastion_ip}\" core@${element(var.master_ip,0)}:/home/core/.kube/config env/$${INSTANCE}/kubeconfig"
  }

  provisioner "local-exec" {
    command = "sed -i \"s:${element(var.master_ip,0)}\\:6443:${var.apiserver_external_dns}\\:${var.apiserver_port}:g\" env/$${INSTANCE}/kubeconfig"
  }
}
